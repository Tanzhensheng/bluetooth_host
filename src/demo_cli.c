...
static int demo_cli_parse_flow_mode(const char *value, mod_ble_flow_mode_t *flow_mode)
{
    if (value == NULL || flow_mode == NULL) {
        return MOD_BLE_STATUS_INVALID_ARG;
    }
    if (strcmp(value, "normal") == 0) {
        *flow_mode = MOD_BLE_FLOW_NORMAL;
        return MOD_BLE_STATUS_OK;
    }
    if (strcmp(value, "abnormal") == 0) {
        *flow_mode = MOD_BLE_FLOW_ABNORMAL;
        return MOD_BLE_STATUS_OK;
    }
    if (strcmp(value, "repair") == 0) {
        *flow_mode = MOD_BLE_FLOW_REPAIR;
        return MOD_BLE_STATUS_OK;
    }
    return MOD_BLE_STATUS_INVALID_ARG;
}
...
void demo_cli_options_init(demo_cli_options_t *options)
{
    ...
    options->config.flow_mode = MOD_BLE_FLOW_NORMAL;
}
...
int demo_cli_parse(int argc, char **argv, demo_cli_options_t *options)
{
    ...
    for (i = 1; i < argc; ++i) {
        if (strcmp(argv[i], "--target") == 0 && (i + 1) < argc) {
            ...
            status = demo_cli_parse_flow_mode(argv[++i], &options->config.flow_mode);
            if (status != MOD_BLE_STATUS_OK) {
                return status;
            }
...
void demo_cli_print_usage(const char *program_name)
{
    ...
    (void)fprintf(stdout,
        ...
        "  [--scan-timeout-ms <ms>] [--recv-timeout-ms <ms>] [--flow normal|abnormal|repair] \\\n"
        "  [--verbose] [payload hex ...]\n",
        name);
}
